...
public class MoveListNet : ScriptableObject
{
    ...
    public void TMANfair(int na)
    {
        p1.state = "attacking";
        p1.lag = 45;
        p1.CmdDeQ(9, .3f, 25, Vector3.right * p1.currentDir*.5f+Vector3.up*.5f, true, Vector3.down*.05f+Vector3.right*p1.currentDir*.05f, -p1.currentDir*.1f,
            p1.thisPlayer, Vector3.up, 10, 0, false, 0, 150, 10);
    ...
    public void TMANdspec(int na)
    {
        ...
        p1.CmdDeQ(15, .5f, 200, Vector3.right * p1.currentDir, false, Vector3.zero, 0,
            0, Vector3.up, 10, 0, false, 0, 150, 10);
    ...
    public void TMANjab(int na)
    {
        ...
        p1.thisPlayer, Vector3.up, 5, 5, false, 0, 50, 10);
        //bool active, int activeOn, float size, int duration, Vector3 location, bool tethered, Vector3 direction, float rotation
        //int playerNum, float angle, int dmg, int sdmg, bool grab, int priority, float bkb, float skb
    }
